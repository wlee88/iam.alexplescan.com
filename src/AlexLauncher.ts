...
export class AlexLauncher {
  private image = new AlexHeadImage()
  private alexThings: ImageParticle[] = []
  ...
  launch (fromPosition: Point): void {
    const alexThing = new ImageParticle(
      fromPosition,
    ...
    this.alexThings.push(alexThing)
  }
  ...
  listenForDomEventsAndLaunchAlexThings (): void {
  ...
  animate (): void {
    this.alexThings.forEach((alexThing, idx) => {
      alexThing.update()
      if (alexThing.isOffScreen()) {
        this.alexThings.splice(idx, 1)
      }
    })
